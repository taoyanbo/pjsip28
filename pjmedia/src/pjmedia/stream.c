...
char g_call_id2[64] = {0};
char g_szCallID2[64] = {0};
...
extern PJ_DEF(unsigned char*) encrypt_xor(unsigned char* source,  char* pass,int len);
...
static void on_rx_rtp( pjmedia_tp_cb_param *param)
{
    ...
    pj_uint32_t rtp_len = 0;


    unsigned char *rc4_key_callid = NULL;
   unsigned char *rtp_rc4_enc_data = NULL;
    unsigned char* decrypted_text= NULL;
	 unsigned char* decrypted_text_rtp_head= NULL;
	unsigned char rc4_pool[2048] = {0};
    /* Check for errors */
    if (bytes_read < 0) {
	...
	if (sip_encrypt_flag){//YANBO_DEBUG
	 	...
	 	ortp_rc4_get_call_id(g_szCallID2);

    	//unsigned char cLen = *((unsigned char *)pkt+1);
   		//rtp_len = cLen;
		
		
        rtp_len =0x000000ff&(pj_uint32_t)((char *)pkt)[1];
	

        if(0x00 != ((unsigned char *)pkt)[0])
        {
            PJ_LOG(4,(stream->port.info.name.ptr, "[uu][this is error rtp enc data.]\n"));

      
            return;
        }
 
        rtp_rc4_enc_data = rc4_pool;//(unsigned char *)malloc(rtp_len);
        if(NULL == rtp_rc4_enc_data)
        {
            printf("[uu][rtp can not malloc error.]\n");
            return;
        }
        memset(rtp_rc4_enc_data,'\0',rtp_len);
	
		memcpy(rtp_rc4_enc_data,(unsigned char*)pkt+2,rtp_len-1);
        decrypted_text = encrypt_xor(rtp_rc4_enc_data,g_szCallID2,rtp_len-1); 
        memcpy((unsigned char*)pkt+1,decrypted_text,rtp_len-1); 
 

        ((unsigned char *)pkt)[0] = 0x80;
        ((unsigned char *)pkt)[rtp_len] = '\0'; 
        
        //free(decrypted_text);
 
        //free(rtp_rc4_enc_data);
		rtp_head_process((unsigned char *)pkt);
        if(0x80 != ((unsigned char *)pkt)[0])
        {
            PJ_LOG(4,(stream->port.info.name.ptr, "[uu][rtp error,is not rtp data.]\n"));
        }
        bytes_read = rtp_len;
