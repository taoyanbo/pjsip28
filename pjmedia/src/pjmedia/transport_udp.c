...
PJ_DEF(unsigned char*) rtp_head_process(unsigned char* source)
{
/*
    int i = 0;
    pjmedia_rtp_hdr *rtp_head = NULL;
    pj_uint16_t seq;
	pj_uint32_t ts;
	rtp_head = (pjmedia_rtp_hdr*)source;
	seq = pj_ntohs((rtp_head)->seq);
	ts = pj_ntohl((rtp_head)->ts);
	PJ_LOG(4, ("trst.c", 
	       "rtp head : seq=%d, timestamp=%u",
	       seq, ts));
...
PJ_DEF(unsigned char*) encrypt_xor(unsigned char* source,  char* pass,int len)
{
    int i = 0;
	static unsigned char encode_buf[2048]={0};
	...
	// 	source_length = 31;
	 source_length = len;
	tmp_str =  &encode_buf[0];//(unsigned char*)malloc((source_length) * sizeof(unsigned char));
    memset(tmp_str, 0, source_length);
 
    for(i = 0; i < source_length; ++i)
    {
        tmp_str[i] = source[i]^pass[0];
...
PJ_DEF(void *) ortp_rc4_set_call_id(char *call_id,int len)
{
	if(NULL != call_id)
 	{
 		
 		memset(g_call_id,'\0',64);
 		strncpy(g_call_id,call_id,len);
 	
 	}

}

PJ_DEF(void *) ortp_rc4_get_call_id(char *call_id)
{
	if(NULL != call_id)
 	{
 		
 	
 		strncpy(call_id,g_call_id,strlen(g_call_id));
				       ...
				       pj_size_t size)
{
    ...
    char *rc4_key_callid = NULL;
    unsigned char *rtp_data_rc4 = NULL;
    unsigned char* decrypted_text= NULL;
    pj_int32_t size_rtp_len = 0;
    unsigned char szRTP_RC4_Buff[2048] = {0};
    /* Must be attached */
    //PJ_ASSERT_RETURN(udp->attached, PJ_EINVALIDOP);
	...
	if (sip_encrypt_flag) {
		size_rtp_len = size;
		ortp_rc4_get_call_id(g_szCallID);

		//rc4_key_callid = (char  *)malloc(strlen(g_szCallID));

		//memcpy(rc4_key_callid,g_szCallID,strlen(g_szCallID));

		rtp_data_rc4 = &szRTP_RC4_Buff[0];//(unsigned char *)malloc(size_rtp_len+2);
		memset(rtp_data_rc4, '\0', size_rtp_len + 2);
		rtp_data_rc4[0] = 0x00;

		decrypted_text = encrypt_xor(&((unsigned char *)pw->buffer)[1], g_szCallID, size_rtp_len - 1);
		memcpy(&rtp_data_rc4[2], decrypted_text, size_rtp_len - 1);

		rtp_data_rc4[1] = (unsigned char)size_rtp_len;



		sent = size_rtp_len + 1;

		pj_memcpy(pw->buffer, rtp_data_rc4, sent);
		...
		/*
			status = pj_ioqueue_sendto( udp->rtp_key,
						...
						udp->addr_len);
						*/
		PJ_LOG(4, (udp->base.name, "[[uu] send rtp,info=%d,%d\n", status, sent));
