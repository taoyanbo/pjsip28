					 ...
					 pjsua_call_id *p_call_id)
{
	...
	if (sip_encrypt_flag)
	{
	 	PJ_LOG(4,(THIS_FILE, "[qwg callid] callid=%.*s\n", dlg->call_id->id.slen,dlg->call_id->id.ptr));
		ortp_rc4_set_call_id(dlg->call_id->id.ptr,dlg->call_id->id.slen);
    }
...
pj_bool_t pjsua_call_on_incoming(pjsip_rx_data *rdata)
{
	...
	if (sip_encrypt_flag) {
		if (dlg->call_id != NULL)
		{
			if (64 <= dlg->call_id->id.slen)
			{
				ortp_rc4_set_call_id(dlg->call_id->id.ptr, 64);
			}
			else
			{
				ortp_rc4_set_call_id(dlg->call_id->id.ptr, dlg->call_id->id.slen);
