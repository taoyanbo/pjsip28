...
pj_str_t * pj_cid = NULL;
	...
	if (sip_encrypt_flag)
    if((NULL != cfg->cred_info[0].data.ptr) && (NULL != cfg->cred_info[0].username.ptr))
    {
       
        belle_sip_rc4_set_key(cfg->cred_info[0].data.ptr); 
        belle_sip_rc4_set_sign_data(cfg->cred_info[0].username.ptr);      
    }
				      ...
				      const pjsua_acc_config *cfg)
{
    ...
    if((NULL != cfg->cred_info[0].data.ptr) && (NULL != cfg->cred_info[0].username.ptr))
    {
        PJ_LOG(4,(THIS_FILE, "[qwg] register passwd=%s,username=%s\n",cfg->cred_info[0].data.ptr,cfg->cred_info[0].username.ptr));
        belle_sip_rc4_set_key(cfg->cred_info[0].data.ptr); 
        belle_sip_rc4_set_sign_data(cfg->cred_info[0].username.ptr);      
    }
