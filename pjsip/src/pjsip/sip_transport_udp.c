...
#if 1 //add by qwg for rc4
typedef struct RC4_ST_HEAD
{
	pj_uint16_t rc4_head_type;
	pj_uint16_t  rc4_head_bl;
	pj_uint16_t rc4_head_sf ;
	pj_uint16_t rc4_data_len;

}RC4_ST_HEAD_S;

extern int sip_encrypt_flag;

char rc4_key[256] = {"654321"};
char rc4_sa[256] = {0};
char rc4_sb[256] = {0};
pj_uint16_t  rc4_head_type = 0x4b53;
pj_uint16_t  rc4_head_bl =  0x00;
pj_uint16_t rc4_head_sf = 0x01;
pj_uint16_t rc4_data_len = 0;
unsigned char rc4_body_sign_type = 0x00;
unsigned char rc4_body_sign_len  = 0;
char rc4_body_sign_data[16] =  {0};

static char *get_rc4_key()
{
	return "";
}
...
void rc4_init_dec(unsigned char *s,unsigned char * s2)
{
	int i = 0;
	for(i=0;i<256;i++)
	{
				  ...
				  pj_ssize_t bytes_read)
{
    ...
    int i;
    pj_status_t status;
   pj_int32_t recv_bufflen = 0;
    char szRevBuff[2048] = {0};
	char szRevBuffTemp[2048] = {0};
	pj_int32_t bs_len = 0;
	pj_int32_t a = 0;
	RC4_ST_HEAD_S stRC4;
	pj_int32_t rc4_recv_data_len = 0;
	pj_int32_t rc4_recv_bs_len = 0;
    ++tp->read_loop_spin;
	...
	if (bytes_read > MIN_SIZE) {
		...
		if (sip_encrypt_flag) {//YANBO_DEBUG
			recv_bufflen = 0x0000ffff & (pj_uint32_t)rdata->pkt_info.len;

			//memcpy(&stRC4,rdata->pkt_info.packet,8);
			PJ_LOG(4, (THIS_FILE, "YANBO UDP RECV END recv buffer len=%d\n", recv_bufflen));
			if (recv_bufflen > 10) {
				memcpy(&stRC4.rc4_head_type, rdata->pkt_info.packet, 2);
				memcpy(&stRC4.rc4_head_bl, &rdata->pkt_info.packet[2], 2);
				memcpy(&stRC4.rc4_head_sf, &rdata->pkt_info.packet[4], 2);
				memcpy(&stRC4.rc4_data_len, &rdata->pkt_info.packet[6], 2);

				if (0x4b53 != ntohs(stRC4.rc4_head_type))
				{
					PJ_LOG(4, (THIS_FILE, "this is not RC4 data of vos,head type error!\n"));
					return;
				}
				if (0x01 == ntohs(stRC4.rc4_head_sf))
				{

				}
				rc4_recv_data_len = ntohs(stRC4.rc4_data_len);



				rc4_init(rc4_sa, (unsigned  char*)rc4_key, strlen(rc4_key));
				rc4_init_dec(rc4_sa, rc4_sb);

				if (recv_bufflen - 8 > sizeof(szRevBuffTemp)) return;

				memcpy(szRevBuffTemp, &rdata->pkt_info.packet[8], (recv_bufflen - 8));



				rc4_recv_bs_len = 0x000000ff & (pj_uint32_t)szRevBuffTemp[1];


				pj_int32_t data_len = recv_bufflen - 8 - 2 - rc4_recv_bs_len;
				if (data_len < 0 || data_len>2048) return;
				memcpy(rdata->pkt_info.packet, &szRevBuffTemp[rc4_recv_bs_len + 2], data_len);


				rc4_crypt(rc4_sb, rdata->pkt_info.packet, data_len);


				memcpy(rdata->pkt_info.packet, rdata->pkt_info.packet, data_len);


				rdata->pkt_info.packet[data_len] = '\0';

				rdata->pkt_info.len = data_len;
				PJ_LOG(4, (THIS_FILE, "YANBO UDP RECV END data_len=%d\n", data_len));
			}
		}
				 ...
				 pjsip_transport_callback callback)
{
    ...
    pj_status_t status;
	    char *pBuff = NULL;
	char *pBuff_send = NULL;
	pj_uint16_t   rc4_head_type2 = 0;//首部标识
	pj_uint16_t   rc4_head_bl2=  0;//首部保留字段
	pj_uint16_t  rc4_head_sf2= 0;//算法标识
	pj_uint16_t  rc4_data_len2 = 0;
	pj_int32_t totaldata_len = 0;//总加密数据的长度
	static unsigned char dataBuffer1[2048]={0};
	static unsigned char dataBuffer2[2048]={0};
	...
	PJ_LOG(4,(THIS_FILE, "[rc4]udp_channel_send buflen=%d", size));

	if (size > 10 && sip_encrypt_flag) {
		belle_sip_rc4_get_key(rc4_key);
		PJ_LOG(4,(THIS_FILE, "[rc4]belle_sip_rc4_get_key key=%s,keylen=%d\n", rc4_key,strlen(rc4_key)));
		rc4_init(rc4_sa,(unsigned  char*)rc4_key,strlen(rc4_key));

		//sip 信令进行加密
		pBuff = dataBuffer1;//(char *)malloc(size);
		memset(pBuff,0,size);

		memcpy(pBuff,tdata->buf.start,size);
		rc4_crypt(rc4_sa,pBuff,size);

	
		memset(rc4_body_sign_data,0,16);
		belle_sip_rc4_get_sign_data(rc4_body_sign_data);

	
		rc4_body_sign_len = strlen(rc4_body_sign_data);

		totaldata_len = 8 + 1 +1 +rc4_body_sign_len +size +1;
		pBuff_send = dataBuffer2;//(char *)malloc(totaldata_len); //tdata->buf.start;
	
		memset(pBuff_send,0,totaldata_len);
		rc4_data_len = strlen(rc4_body_sign_data) + size +2;
		rc4_head_type2 = htons(rc4_head_type);
		rc4_head_bl2 = htons(rc4_head_bl);
		rc4_head_sf2 = htons(rc4_head_sf);
		rc4_data_len2 = htons(rc4_data_len);
		memcpy(pBuff_send,&rc4_head_type2,2);
		memcpy(pBuff_send+2,&rc4_head_bl2,2);
		memcpy(pBuff_send+4,&rc4_head_sf2,2);
		memcpy(pBuff_send+6,&rc4_data_len2,2);
		memcpy(pBuff_send+8,&rc4_body_sign_type,1);
		memcpy(pBuff_send+9,&rc4_body_sign_len,1);
		memcpy(pBuff_send+10,rc4_body_sign_data,rc4_body_sign_len);
		memcpy(pBuff_send+10+rc4_body_sign_len,pBuff,size);

		if (totaldata_len < tdata->buf.end - tdata->buf.start) 
		{		
			memcpy(tdata->buf.start,pBuff_send, totaldata_len );	
			tdata->buf.cur=tdata->buf.start + totaldata_len;
		}else {
			PJ_LOG(4,(THIS_FILE, "YANBO send buffer not enough max%d,size %d\n", (tdata->buf.end - tdata->buf.start),totaldata_len));
			//tdata->buf.start = (char *)pj_pool_alloc(tdata->pool,totaldata_len);
			//memcpy(tdata->buf.start,pBuff_send, totaldata_len );
			//tdata->buf.end = tdata->buf.start + totaldata_len;
			//tdata->buf.cur=tdata->buf.start + totaldata_len;
		}
  		...
  		status = pj_ioqueue_sendto(tp->key, (pj_ioqueue_op_key_t*)&tdata->op_key,
			       tdata->buf.start, &totaldata_len, 0,
			       rem_addr, addr_len);
				   PJ_LOG(4,(THIS_FILE, "pj_ioqueue_sendto status=%d\n", status));
	}
	else
	//YANBO_DEBUG	
 		status = pj_ioqueue_sendto(tp->key, (pj_ioqueue_op_key_t*)&tdata->op_key,
