...
static char szRc4Key[256] = {"123456"};
static char rc4_body_sign_data[16] = {0};

PJ_DEF(void) belle_sip_rc4_set_key(char *pKey)
 {
    if(NULL == pKey)
    {
        return;
    }
    strncpy(szRc4Key,pKey,strlen(pKey));
    PJ_LOG(4,(THIS_FILE, "[rc4]rc4_set_key %s",szRc4Key));
    return;
 }

PJ_DEF(void) belle_sip_rc4_get_key(char *pKey)
 {
    memcpy(pKey,szRc4Key,256);

    return;
 }

 PJ_DEF(void)  belle_sip_rc4_set_sign_data(char *pData)
 {
    if(NULL == pData)
    {
        return;
    ...
    strncpy(rc4_body_sign_data,pData,strlen(pData));

    PJ_LOG(4,(THIS_FILE, "[rc4]rc4_body_sign_data set:%s",rc4_body_sign_data));
    return;
 }

PJ_DEF(void)  belle_sip_rc4_get_sign_data(char *pData)
 {
    memcpy(pData,rc4_body_sign_data,strlen(rc4_body_sign_data));
